# src/models/xgb_fe_optuna.py
"""
Modelo XGBoost con optimización de hiperparámetros usando Optuna para predicción de fallas.
Utiliza datos con feature engineering y se enfoca en maximizar la detección de fallas.
"""
...
STORAGE = f"sqlite:///{(PIPE / 'optuna_study_fe.db').as_posix()}"
...
def custom_failure_score(y_true, y_pred, y_proba=None):
    """
    Métrica personalizada que prioriza la detección de fallas.
    Combina F2-score (prioriza recall) con penalización por falsos positivos.
    """
    recall = recall_score(y_true, y_pred, zero_division=0)
    precision = precision_score(y_true, y_pred, zero_division=0)
    
    # F2-score (da más peso al recall)
    f2 = (1 + 2**2) * (precision * recall) / (2**2 * precision + recall) if (2**2 * precision + recall) > 0 else 0
    
    # Penalización por falsos positivos (queremos minimizar impacto en clase 0)
    fp_penalty = 0
    if y_proba is not None:
        # Penalizar predicciones con alta confianza para clase 0
        false_positive_confidence = np.mean(y_proba[y_true == 0])
        fp_penalty = false_positive_confidence * 0.1  # Factor de penalización
    
    # Score final: F2-score menos penalización por falsos positivos
    final_score = f2 - fp_penalty
    
    return final_score, f2, recall, precision
...
def objective(trial: optuna.Trial):
    ...
    Se enfoca en maximizar la detección de fallas con mínima penalización en no-fallas.
    ...
    Retorna:
    --------
    float : Score personalizado que prioriza detección de fallas
    """
    ...
    for thresh in thresholds:
        ...
        custom_score, f2, recall, precision = custom_failure_score(y_val, y_pred_thresh, proba)
    ...
    trial.set_user_attr("f2_score", best_f2)
...
if __name__ == "__main__":
    ...
    print("Objetivo: Maximizar detección de fallas con mínima penalización en no-fallas")
    ...
    print(f"  F2-score: {best_trial.user_attrs['f2_score']:.4f}")
    ...
    for thresh in thresholds:
        ...
        custom_score, f2, recall, precision = custom_failure_score(y_val, y_pred_thresh, proba_val)
    ...
    # Métricas en test
    custom_score_test, f2_test, rec_test, prec_test = custom_failure_score(y_test, y_pred_test, proba_test)
    ...
    model_path = MODEL_DIR / "xgb_fe_optuna_best.joblib"
    joblib.dump(final_model, model_path)
    print(f"Modelo guardado en: {model_path}")
    
    # Guardar estudio
    study_path = MODEL_DIR / "xgb_fe_optuna_study.joblib"
    ...
    threshold_path = MODEL_DIR / "xgb_fe_optuna_threshold.json"
    ...
    print(f"  - F2-score de {f2_test:.4f} (prioriza detección de fallas)")
    print(f"  - Umbral optimizado: {thr:.4f}")
